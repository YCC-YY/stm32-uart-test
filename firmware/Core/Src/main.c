...
/* USER CODE BEGIN 0 */
#define BUF_LEN 64
uint8_t recv_byte;       // HAL库IT接收缓冲区，不要volatile
...
int main(void) {
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  uint8_t send_buf[] = "Hello STM32 Test!\r\n";
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  if(huart->Instance == USART1)
  {
    if(recv_byte == '\n')   // 收到换行符，表示命令结束
    {
      new_data_flag = 1;   // 通知主循环处理完整命令
    }
    else if(recv_byte != '\r')  // 忽略回车，只保留有效字符
    {
      if(rx_idx < BUF_LEN - 1)
      {
        rx_buf[rx_idx++] = recv_byte;
      }
    }
    HAL_UART_Receive_IT(&huart1, &recv_byte, 1);
  }
